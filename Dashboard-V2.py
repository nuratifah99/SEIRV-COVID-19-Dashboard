...
tabs = ["Home","Simulation", "Dashboard"]
...
if active_tab == "Home":
    ...
    st.write("This page was established for academic purpose only.")
    
elif active_tab == "Simulation":
    title="SEIRV Model for COVID-19"
    ...
    # fig_data_V.update_layout(width=400,height=300)
    ...
    fig_model=go.Figure()
        # fig.update_layout(yaxis_range=[0,10000000])
    ...
    subtitle="COVID-19 situation in Malaysia based on SEIRV Model"
    st.markdown(f"<h2 style='text-align: center; color: black;'>{title}</h2>", unsafe_allow_html=True)
    st.markdown(f"<h4 style='text-align: center; color: black;'>{subtitle}</h4>", unsafe_allow_html=True)
    ...
    #     Eobv=data[:, 0]
    #     Efit=fitted_predicted[:, 1]
    #     E_data=np.array([Eobv,Efit])
    #     E_trans=np.transpose(E_data)
    #     E_plot=pd.DataFrame(E_trans[:,:],t,columns=['Observe Exposed', 'Fitted Exposed'])
    #     st.line_chart(E_plot,width=1000) 
    ...
    with chart02:
    #     st.markdown(f"<h3 style='text-align: center; color: black;'>{'SEIRV Model'}</h3>", unsafe_allow_html=True)
        st.plotly_chart(fig_model, use_container_width=False)
    #     fitted_predicted=np.around(fitted_predicted)
    #     fitted_predicted=fitted_predicted.astype(int)
#     t=pd.DataFrame({'Date':t})
#     S=pd.DataFrame({'Susceptible':fitted_predicted[:, 0]})
#     E=pd.DataFrame({'Exposed':fitted_predicted[:, 1]})
#     I=pd.DataFrame({'Infected':fitted_predicted[:, 2]})
#     R=pd.DataFrame({'Removed':fitted_predicted[:, 3]})
#     V=pd.DataFrame({'Vaccinated':fitted_predicted[:, 4]})
    
#     combine_data=pd.concat([t,S,E,I,R,V],join='inner',axis=1)
#     t=combine_data['Date'].dt.date
#     fitted_data=pd.concat([t,S,E,I,R,V],join='inner',axis=1)
    
#     st.dataframe(fitted_data)
    
# with chart03:
#     figu=px.line(fitted_data,x='Date',y='Susceptible',animation_frame='Date', animation_group='Susceptible')  
#     figu.show()
# st.markdown("<hr/>",unsafe_allow_html=True)
